...
function getAstroPagePath({ routeType, slug, segmentKey }) {
  ...
  return path.join(process.cwd(), "src", "pages", "[lang]", slug, "index.astro");
}
...
function getTemplatePath(routeType) {
  const name = routeType === "segment" ? "newpage-segment.astro" : "newpage-non-segment.astro";
  return path.join(process.cwd(), "scripts", "templates", name);
}
...
async function writeSeoJsonFiles({ slug, seoTitleFr, seoDescFr, seoTitleEn, seoDescEn }) {
...
async function updateSegments({ segmentKey, segmentFr, segmentEn }) {
  const segmentsPath = path.join(process.cwd(), "src", "helpers", "segments.js");
  ...
  const extractEntry = (text) => {
    const entryMatch = text.match(
      new RegExp(`${segmentKey}:\\s*\\{\\s*fr:\\s*\"([^\"]+)\",\\s*en:\\s*\"([^\"]+)\"\\s*\\}`)
    );
  ...
  if (updatedBlock === block) {
    updatedBlock = block.replace("const segments = {\n", `const segments = {\n${insertion}`);
  }
  if (updatedBlock === block) {
    throw new Error("Failed to insert new segment. Check segments.js formatting.");
  }
...
async function updateMenu({ locale, navPlacement, label, href }) {
  const menuPath = path.join(process.cwd(), "public", "content", locale, "menu.json");
  ...
  const showIn =
    navPlacement === "both" ? ["header", "footer"] : navPlacement === "none" ? [] : [navPlacement];
...
async function main() {
  ...
  if (navPlacement !== "none") {
    await updateMenu({ locale: "fr", navPlacement, label: labelFr, href: frHref });
    await updateMenu({ locale: "en", navPlacement, label: labelEn, href: enHref });
  }
